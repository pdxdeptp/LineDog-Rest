...
Continue with **R7: Move learning capacity Hermes sync out of defaults** when settings/defaults work is explicitly in scope. **R6: Move dashboard layout clamp policy out of defaults** is complete; `DashboardLayout.swift` now owns dashboard column width and left-plan fraction policy while `MalDazeDefaults` preserves the old key/API names as compatibility wrappers.
