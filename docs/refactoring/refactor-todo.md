...
Continue with **OpenSpec task 3.5: Inject shared Hermes path helpers into contract readers** only when Hermes contract reader work is explicitly in scope. **R4: Extract Hermes path and process runtime helpers** is complete for CLI runtime consolidation; **R5: Split defaults key namespaces from behavior** should wait until remaining Hermes runtime tasks are handled or intentionally deferred.
