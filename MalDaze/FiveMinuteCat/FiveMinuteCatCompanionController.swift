...
final class FiveMinuteCatCompanionController {
    ...
    private func observeDogFrame() {
        frameObserver = NotificationCenter.default.addObserver(
            forName: MalDazeBroadcastNotifications.idlePetScreenFrameChanged,
            object: nil,
            queue: .main
        ) { [weak self] note in
            Task { @MainActor [weak self] in
                guard let self,
                      let v = note.userInfo?[MalDazeBroadcastNotifications.idlePetScreenFrameUserInfoKey] as? NSValue
                else { return }
