...
学习助手支持从学习资料 URL 生成结构化学习计划草稿。当前实现通过 FastAPI 调用 LangGraph Ingestion Agent：`POST /api/ingest/start` 立即返回 `thread_id`，分析进度由 `GET /api/ingest/progress/{thread_id}` 以 SSE 推送；代理识别资料类型、抽取学习单元、估算工时、生成两个排期方案，经 `present_draft` 中断待用户确认后写入本地数据库。
