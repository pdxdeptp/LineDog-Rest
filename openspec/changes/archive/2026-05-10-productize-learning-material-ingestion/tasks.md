...
- [x] 5.3 手动验收：B站合集 27 视频 → SSE 四阶段正常 → 草稿显示"均匀铺开"为默认 → 弹窗展示 27 天排期 → 修改 deadline → 重新排期 → 确认写入 → 首页刷新
- [x] 5.4 手动验收：无效 URL → error 事件 → 友好错误提示，URL 文本保留
- [x] 5.5 手动验收：学习偏好设置页读取/修改 `daily_capacity_min`，Ingestion 草稿卡片显示正确值
- [x] 5.6 手动验收：取消草稿 → URL 不丢失 → DB 无写入
