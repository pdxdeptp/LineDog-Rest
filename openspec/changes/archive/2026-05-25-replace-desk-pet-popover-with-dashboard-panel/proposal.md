...
- Preserve the existing wide three-column dashboard concept: reminders, retired middle-column feature, and timer/pet controls.
- Show cached dashboard content immediately when available and refresh in the background instead of forcing the retired middle-column feature column through a gray startup state on every open.
...
- `dashboard-panel-ui`: The wide dashboard is hosted in a reusable panel context with cached-content startup behavior and background refresh expectations.
...
- `MalDaze/LearningAssistant/retired middle-column view`: continue rendering the retired middle-column feature column, but participate in cached dashboard startup and refresh behavior.
