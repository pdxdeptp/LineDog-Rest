...
- [x] 6.2 手动测试：开启喝水提醒，将间隔改为 1 分钟（最小 15 分钟不符，可临时改代码下限为 1），等待浮层弹出，验证「已喝水」与「稍后提醒」各自行为正确
- [x] 6.3 手动测试：关闭喝水提醒开关，确认不再弹出浮层；重启应用，确认开关状态保留
- [x] 6.4 手动测试：间隔 Stepper 步进，确认值在 15–240 之间变化；重启应用确认间隔持久化
- [x] 6.5 手动测试：SevenMinuteReminder 铃铛与喝水浮层同时显示（时机调好），确认两个窗口互不干扰
