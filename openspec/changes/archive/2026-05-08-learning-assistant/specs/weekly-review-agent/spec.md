...
### Requirement: 周日定时触发
系统 SHALL 通过 APScheduler 在每周日 20:00 触发 Weekly Review Agent。若后端进程在触发时刻未运行，SHALL 在下次 Morning Agent 启动时补触发（通过查询 events 表判断，不使用 flag 字段）。
...
#### Scenario: 后端离线补触发
- **WHEN** 周日 20:00 时后端进程未运行（APScheduler 无法触发）
- **THEN** 下次 Morning Agent 启动时，查询 events 表确认上周日是否存在 `weekly_review_done` 事件；若不存在，Morning Agent Graph 优先执行 Weekly Review 子图，完成后继续生成今日摘要
...
### Requirement: 任务调度范围限制
Weekly Review Agent SHALL NOT INSERT 新任务行。其唯一权限是 UPDATE 现有 tasks 的 `scheduled_date`、`priority`、`status`。新任务的创建权归属于 Ingestion Agent（导入资料时）和 Conversational Planner（用户明确要求时）。

#### Scenario: 重排而非新增
- **WHEN** Weekly Review Agent 决定下周某天负担过重
- **THEN** 系统通过调整现有任务的 `scheduled_date` 来重新分配，不新增任务行
