...
This change reduces idle CPU wakeups and battery impact while preserving the existing desktop pet interactions, rest interruption flows, and retired middle-column feature behavior.
...
- Add a user setting for local retired middle-column feature backend startup mode: lazy startup when the assistant UI needs it, or eager startup at app launch for users who prefer lower first-open latency.
...
- `dashboard-panel-ui`: The retired middle-column feature backend startup mode must be user-configurable; lazy mode starts on assistant UI activation, eager mode starts at app launch, and the panel must still show connecting/offline/ready states correctly.
