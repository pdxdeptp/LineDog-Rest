...
- [x] 2.1 Add activation event recording linking intake item, assumptions, draft schedule version, and created active tasks.
- [x] 2.2 Reject stale draft activation without creating active task rows.
- [x] 2.3 Reject activation when the requested draft version lacks activation-ready task data and schedule slices.
- [x] 2.4 Ensure activation is transactional: no partial active resource/unit/task/event rows remain after failure.
- [x] 2.5 Ensure draft cancellation/discard does not change active Today or Calendar facts.
- [x] 2.6 Reject invalid lifecycle transitions without changing prior state or active tasks.
- [x] 2.7 Ensure existing-plan draft activation appends units/tasks under the target plan without creating a new top-level resource.
- [x] 2.8 Make duplicate activation idempotency-safe without duplicate active rows.
- [x] 2.9 Reject discard/cancel after activation without mutating active plan rows.
...
- [x] 4.4 Add stale activation rejection tests.
- [x] 4.5 Add activation event persistence and transaction rollback tests.
...
- [x] 4.13 Add discard-after-activation rejection tests.
