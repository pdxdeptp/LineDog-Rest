...
### Requirement: 归一化资料结构
所有 Handler SHALL distinguish preview metadata from confirmed legacy resource structures so Add / Initiate can inspect sources without forcing every source into scheduled units.
...
- **THEN** 系统优先从 README 中提取有序结构或预览 outline
...
- **WHEN** README 与目录树都不能提供可靠结构 and the request is an Add / Initiate preview
...
#### Scenario: GitHub repo facts are unavailable
- **WHEN** README, topics, description, or directory signals are unavailable
- **THEN** ingestion leaves those fields unknown
- **AND** it does not invent repository structure or source facts

### Requirement: Plan-Generating Material Handoff
Material ingestion SHALL hand parsed source structure to the plan compiler only after the user confirms a plan-generating role.

#### Scenario: User confirms source as plan-generating
- **WHEN** the user confirms a source as a new plan, clone/rebuild target, or existing-plan attachment with scheduled work
- **THEN** material ingestion provides source structure and rough duration signals to the plan compiler
- **AND** the plan compiler remains responsible for output-driven phases, daily schedule, buffer, and risk states

#### Scenario: Source structure conflicts with target output
- **WHEN** parsed source units do not match the user's target output or target depth
- **THEN** the plan compiler prioritizes the confirmed target output and depth
- **AND** it may omit, merge, or mark source sections as optional rather than scheduling every parsed unit
